...
/**
 * Builds a JSON with Binary attachments:
...
public:
	...
	/**
	 ...
	 If want to simplify just pass a pointer and let the JSONBinBuilder copy the memory
	 ...
	 * This variant takes a filename. If the deferload is true or we have base64 we'll load the file immediately
		Alternatively we'll load it at serialization time (for JSONBin) or expose it via link (for post streaming)
